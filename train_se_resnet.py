...
This implementation is based on ResNet v2, which come from 
Kaiming He, Xiangyu Zhang, Shaoqing Ren, Jian Sun. "Identity Mappings in Deep Residual Networks" 

Coded by Lin Xiong Sep-25, 2017
"""
import argparse,logging,os
import mxnet as mx
from symbol_se_resnet import resnet
...
def main():
    ...
    if args.data_type == "cifar10":
        ...
        units = per_unit*3
        symbol = resnet(units=units, num_stage=3, filter_list=filter_list, ratio_list=ratio_list, num_class=args.num_classes,
        ...
        symbol = resnet(units=units, num_stage=4, filter_list=[64, 256, 512, 1024, 2048] if args.depth >=50
                        else [64, 64, 128, 256, 512], ratio_list=ratio_list, num_class=args.num_classes, data_type="imagenet", bottle_neck = True
    ...
    if not os.path.exists("./model"):
        os.mkdir("./model")
    model_prefix = "model/se-resnet-{}-{}-{}".format(args.data_type, args.depth, kv.rank)
    ...
    train = mx.io.ImageRecordIter(
        path_imgrec         = os.path.join(args.data_dir, "train.rec") if args.data_type == 'cifar10' else
                              os.path.join(args.data_dir, "msceleb1m_face_clean_train_256_q90.rec") if args.aug_level == 1
                              else os.path.join(args.data_dir, "msceleb1m_face_clean_train_384_q90.rec") ,
        label_width         = 1,
        data_name           = 'data',
    ...
    val = mx.io.ImageRecordIter(
        path_imgrec         = os.path.join(args.data_dir, "val.rec") if args.data_type == 'cifar10' else
                              os.path.join(args.data_dir, "msceleb1m_face_clean_val_256_q90.rec"),
        label_width         = 1,
        data_name           = 'data',
        label_name          = 'softmax_label',
        batch_size          = args.batch_size,
...
if __name__ == "__main__":
    ...
    hdlr = logging.FileHandler('./log/log-se-resnet-{}-{}.log'.format(args.data_type, args.depth))
